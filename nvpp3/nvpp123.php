...
header('Content-Type: application/json');
        ...
        switch($_POST['functionname'])
        {
            case 'aNVrtnpp1row':
                if( !is_array($_POST['arguments']) || (count($_POST['arguments']) < 2) ) {
                    ...
                    $aResult['result'] = aNVrtnpp1row($_POST['arguments'][0], $_POST['arguments'][1]);
            ...
            default:
               $aResult['error'] = 'test.php: Not found, function '.$_POST['functionname'].'!';
               break;
        }
    ...
    echo json_encode($aResult);
      ...
      //============= BEGIN aNVrtnpp1row($values, $blank) ===============================
      function aNVrtnpp1row($values, $blank)
      {
         $obj = new User;
         //$myRetnArray = [];
         //$myRetnArray = $obj->getAllUsers($values);      
         //return $myRetnArray;   // "returning";  // 
         //$obj->getaAllUsers($values);
         return  $obj->getaAllUsers($values);                 //$rows;
         // return Db::connect();
     }
     //============= END aNVrtnpp1row($values, $blank) =================================
     
     //============= BEGIN anvUpdatePP($values, $blank) ===============================
     function anvUpdatePP($values, $blank)
     {
      $obj = new User;
      return $obj->aupdateProfilePage($values);
